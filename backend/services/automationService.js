...
async function getAutomation() {
  const rows = await db.query('SELECT * FROM automations ORDER BY id DESC LIMIT 1');
...
async function saveAutomation(data) {
  ...
  if (existing) {
    await db.query('UPDATE automations SET enabled = ?, steps_json = ?, cooldown_hours = ?, trigger_mode = ? WHERE id = ?', [
      data.enabled ? 1 : 0,
      stepsJson,
  ...
  const result = await db.query('INSERT INTO automations (enabled, steps_json, cooldown_hours, trigger_mode) VALUES (?, ?, ?, ?)', [
    data.enabled ? 1 : 0,
    stepsJson,
    data.cooldown_hours || 24,
    triggerMode
...
function getRunDecision(automation, context = {}) {
  ...
  return {
    allowed,
    reason: allowed ? 'cooldown_elapsed' : 'cooldown_waiting'
  };
}
...
function getStepText(step) {
  ...
  if (step.type === 'image') return (step.caption || '').trim() || null;
  return null;
}

async function scheduleAutomationSteps(chatId, conversationId, steps, initialDelaySeconds = 0) {
  ...
  for (const step of steps || []) {
    ...
    if (!['text', 'image', 'audio'].includes(step.type)) continue;
    if (step.type === 'text' && !step.text) continue;
    if ((step.type === 'image' || step.type === 'audio') && !step.media_id) continue;

    const scheduledAt = new Date(Date.now() + (delaySeconds * 1000));
    await db.query(
      'INSERT INTO scheduled_messages (conversation_id, chat_id, type, text, media_id, scheduled_at, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())',
      [conversationId, chatId, step.type, getStepText(step), step.media_id || null, scheduledAt, 'pending']
    );
    scheduledCount += 1;
  }

  if (scheduledCount) {
    await logService.create('info', 'automation_scheduled', `scheduled ${scheduledCount} follow-up steps for ${chatId}`);
  }
}
...
async function processScheduledMessages(sendText, sendMediaById) {
  ...
  for (const row of rows) {
    try {
      ...
      if ((row.type === 'image' || row.type === 'audio') && row.media_id) {
        await sendMediaById(row.chat_id, row.media_id, row.type, row.type === 'image' ? row.text : null);
      }
      await db.query('UPDATE scheduled_messages SET status = ?, updated_at = NOW() WHERE id = ?', ['sent', row.id]);
      await logService.create('info', 'scheduled_sent', `scheduled ${row.type} sent to ${row.chat_id}`);
    } catch (error) {
      await db.query('UPDATE scheduled_messages SET status = ?, updated_at = NOW() WHERE id = ?', ['failed', row.id]);
      await logService.create('error', 'scheduled_send_failed', `failed scheduled ${row.type} to ${row.chat_id}: ${error.message}`);
    }
  }
  return rows.length;
}
...
async function runAutomation(sendText, sendMediaById, chat, conversationId, context = {}) {
  ...
  if (!decision.allowed) {
    await logService.create('info', 'automation_skipped', `${decision.reason} for ${chat}`);
    return false;
  }

  await logService.create('info', 'automation_started', `${decision.reason} for ${chat}`);
  ...
  for (let index = 0; index < automation.steps.length; index += 1) {
    ...
    if (step.type === 'delay' && delaySeconds > 0) {
      ...
      await scheduleAutomationSteps(chat, conversationId, pendingSteps, delaySeconds);
      await logService.create('info', 'automation_delay', `scheduled follow-up for ${chat} after ${delaySeconds} seconds`);
      break;
    }
    ...
    try {
      ...
      if (sent) {
        await logService.create('info', 'automation_step', `sent ${step.type} to ${chat}`);
      }
    } catch (err) {
      const stepLabel = step.media_id ? `${step.type} media ${step.media_id}` : step.type;
      await logService.create('error', 'automation_error', `${stepLabel}: ${err.message || String(err)}`);
    }
  }
